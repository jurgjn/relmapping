...
rule scap814: # dependent on pooled .bam-files, as generated by target 'scap814' in workflows/scap814_pool.snakefile
    input:
        expand(pf('scap814_{sample}', 'tg_se.bwa_se.rm_unmapped.rm_chrM.rm_blacklist.rm_q10.firstbp_fwd', '.bw', 'scap814'), sample=techreps_collapse(config['scap814'].keys())),
        expand(pf('scap814_{sample}', 'tg_se.bwa_se.rm_unmapped.rm_chrM.rm_blacklist.rm_q10.firstbp_rev', '.bw', 'scap814'), sample=techreps_collapse(config['scap814'].keys())),
...
rule scap815_sum_all:
    input:
        expand(pf('scap815_{stage}', '{{step}}.sum_by_stage', '.bw', 'scap815'), stage=config['stages_wt']),
    output:
        pf('scap815_all', '{step}.sum_all', '.bw', 'scap815'),
    shell: '''
        scripts/bigWiggleTools.ipy write_bg {output[0]} sum {input[0]} {input[1]} {input[2]} {input[3]} {input[4]} {input[5]}
    '''
...
rule bw_gt1:
    input:
        pf('{bid}', '{step}', '.bw', '{prefix}'),
    output:
        pf('{bid}', '{step}.gt1', '.bw', '{prefix}'),
    shell: '''
        scripts/bigWiggleTools.ipy write_bg {output[0]} mult {input[0]} gt 1 {input[0]}
    '''

rule scap815_gt0x2:
    input:
        expand(pf('scap815_{stage}', '{{step}}', '.bw', 'scap815'), stage=config['stages_wt_rep'] + ['wt_emb_rep3', 'wt_emb_rep4']),
    output:
        pf('scap815_all', '{step}.gt0x2', '_sum.bw', 'scap815'), # sum of signal across all replicates
        pf('scap815_all', '{step}.gt0x2', '_gt0_count.bw', 'scap815'), # number of replicates with non-zero signal
        pf('scap815_all', '{step}.gt0x2', '.bw', 'scap815'), # summed signal from base pairs with non-zero coverage in at least two replicates
    shell: '''
...
rule scap815:
    input:
        ...
        pf('scap815_all', 'tg_se.bwa_se.rm_unmapped.rm_chrM.rm_blacklist.rm_q10.firstbp_fwd.sum_all', '.bw', 'scap815'),
        pf('scap815_all', 'tg_se.bwa_se.rm_unmapped.rm_chrM.rm_blacklist.rm_q10.firstbp_rev.sum_all', '.bw', 'scap815'),
        pf('scap815_all', 'tg_se.bwa_se.rm_unmapped.rm_chrM.rm_blacklist.rm_q10.firstbp_rev.sum_all.neg', '.bw', 'scap815'),
        pf('scap815_all', 'tg_se.bwa_se.rm_unmapped.rm_chrM.rm_blacklist.rm_q10.firstbp_fwd.gt0x2', '.bw', 'scap815'),
        pf('scap815_all', 'tg_se.bwa_se.rm_unmapped.rm_chrM.rm_blacklist.rm_q10.firstbp_rev.gt0x2', '.bw', 'scap815'),
        pf('scap815_all', 'tg_se.bwa_se.rm_unmapped.rm_chrM.rm_blacklist.rm_q10.firstbp_rev.gt0x2.neg', '.bw', 'scap815'),
